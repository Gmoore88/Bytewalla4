...
public class DTNManager extends Activity  {
	...
	private Button editConfigMenuButton;
	/**
...
//				.findViewById(R.id.DTNManager_RouteLocalEIDTextView);
	...
	private void init_buttons() {		
		...
		editConfigMenuButton = (Button) this
				.findViewById(R.id.DTNManager_Config);
		editConfigMenuButton.setOnClickListener(new OnClickListener() {
		
			
			public void onClick(View v) {
				Intent i = new Intent(DTNManager.this, DTNConfigMenu.class);
				startActivity(i);
	...
	private void start_DTN_service() throws FileNotFoundException,
		...
		InputStream in=null;
		try {
			//in = this.getAssets().open(getResources().getString(R.string.DTNConfigFilePath));
			in = openFileInput(getResources().getString(R.string.DTNConfigFilePath));
		} catch (IOException e) {
			// TODO Auto-generated catch block
			e.printStackTrace();
		}
		dtn_config_ = DTNConfigurationParser
				.parse_config_file(in, myPrefs);
...
//		  SharedPreferences myPrefs = this.getSharedPreferences("myPrefs", MODE_WORLD_READABLE);
//		  TextView Q_Value = (TextView) this.findViewById(R.id.DTNManager_Q_Value_TextView);
//		  Q_Value.setText(String.format("Q Value: %.2f",(double)myPrefs.getInt("Q_Value", 50)/100));
