...
public class DTNConfigurationParser {
	 ...
	 * @throws Exception
	 */
	private static void process_config_element(Element config_element, DTNConfiguration config, SharedPreferences myPrefs) throws Exception
	{
		...
		{
			process_routes_config(config_element, config, myPrefs);
			return;	
		}
	 ...
	 * @throws InvalidDTNConfigurationException
	 */
	private static void process_routes_config(Element config_element, DTNConfiguration config, SharedPreferences myPrefs) throws InvalidDTNConfigurationException
	{
		NodeList childNodes = config_element.getChildNodes();
		...
		else if (router_type.equals("prophet"))
		{
			Log.i("Bytewalla4", String.format("Router Type: %s",router_type));
			config.routes_setting().set_router_type(router_type_t.PROPHET_BUNDLE_ROUTER);
		}
		else if (router_type.equals("epidemic"))
		{
			Log.i("Bytewalla4", String.format("Router Type: %s",router_type));
			config.routes_setting().set_router_type(router_type_t.EPIDEMIC_BUNDLE_ROUTER);
		}
